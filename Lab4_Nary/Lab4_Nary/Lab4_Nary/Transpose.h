...
public:
	...
	// Clears the list
	void reorder(const E& it, bool found) { //Reorders the list in terms of Transpose
		if (found == true && list.getCurrent() != list.getHead()) {
			E temp = list.getValue(); // Gets the value within that list
			int tempFreq = list.getFreq();
			list.remove();
			list.prev();
			list.insert(temp);
			list.setFreq(tempFreq);
		}
	}
